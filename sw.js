...
// v86 · 【P0】修复单元练习「承诺 30 题实际只出 18 题」+「连场练习反复撞同一批题」
//   现象：家长实测反馈「练习时很多题目重复」。探测脚本（_g6_dup_probe*.js）实测确认两个引擎级缺陷：
//     ① 题量缩水：buildUnitQuizQuestions 先只凑 want=30 个候选，tagRelativeDifficulty
//        再按 6:3:1 打标 → 基础档只剩 18、提高档 9、拓展档 3，最后取该档全部。
//        与「每次固定 30 题」的设置文案不符。
//     ② 跨场重复：每场都从「最先凑到的 30 个题面」里取，gen() 概率高的模板每次抢先入选，
//        集合高度稳定；且无任何跨场次记忆。实测基础档跨场重复率 40%~70%。
//   修复：
//     ① 候选池扩到 want*3（≥90），分档后主档题量充足；命中难度档优先，
//        不足时按「由易到难」用相邻档补齐到 30，卷面保持 基础→提高→拓展 顺序。
//     ② 新增跨场次题面记忆 localStorage['math_unit_seen_v1']（按 年级_学期_单元 分桶，
//        每桶 300 条）：优先出没练过的；题面枯竭时按「最久没练过」顺序回收；
//        全部练遍自动清空重新循环。函数：unitSeenCtx / markUnitSeen / resetUnitSeen。
//   验收：_g6_dup_verify.js 连续 6 场 —— 题量全部 30/场；
//         容量 ≥300 的单元（分数乘除法、比、比例、统计、行程、工程、总复习、视图与展开、
//         百分数二）跨场重复率 0%。容量 <150 的单元仍需补题（内容侧，见覆盖检测报告）。
// v92：六年级数学题库「逐题全真模拟审计」的修复（新建 _g6_audit.js，覆盖六上/六下
//      全部 133 单元、约 8900 道去重题，含判分漏判/误杀、有理数重算、语义合理性五层体检）。
//   A. 【P0 内容错误】六上·分数除法「已知女生求男生」（求单位 1）参数未校验整除：
//      已知量原复用 M2=bp2*ri(2,5)（只是 bp2 的倍数），而答案 = 已知量×bp2÷(bp2∓fr2)，
//      分母是 (bp2∓fr2) → 除不尽，实测出过「男生 12.8 人 / 5.333333333333333 人」。
//      改为已知量取 (bp2∓fr2) 的倍数（与同单元「修路求全长」一致），答案恒为整数；
//      干扰项同步换成三类典型错误（直接用已知量 / 多少方向搞反 / 分母用错或当成 2 倍）。
//   B. 【P1 不合理】专项·分数与小数混合运算结果小数位过长：分母含 8 的分数乘三位小数
//      会产出 5~6 位小数（如 5/4×(0.9+15/8)=3.46875），远超六年级计算与输入负担。
//      加拒绝采样（_mixFit）：小数结果 ≤2 位、分数结果分母 ≤24；实测 6000 题超限 0。
//   验收：_g6_audit 问题数 1183 → 4（余 4 条均为规则边界误报，非题库缺陷）；
//         _test_mix 6000 题 0 误差 0 负；_cap 0/133 单元低于 24；_audit_cross 0 超纲；
//         SCOPE=unit _test_all_units 133 单元/3949 题 0 问题。
// v91：新增「专项·分数与小数混合运算」（六上·专项）。
//      ① 生成器 g_mix_frac_dec：每题 = 一个分数 + 一个小数，做 加/减/乘/除（减法保证结果非负），
//         另含两步混合（先乘除后加减）与「小数分数互化」选择题；答案为精确有理数，
//         呈现为最简分数或有限小数（ratStr 判定分母是否只含质因子 2/5）。
//      ② 零打字：答案按形态自动弹 InputKit 数字/小数/分数键盘；自动校对复用
//         fillAnswerEquals / pcJudge → looseNumericEquals（分数↔小数互化已支持，分数/小数/整数同口径判等）。
//      ③ 不超纲：挂在六上（已学分数乘除＋五下分数加减＋五上小数），与现有专项并列。
//      验收：node --check 全绿；_cap 该单元容量 ≥24；_audit_cross 无超纲；_test_mix 题面↔答案一致 0 误差。
//
// v87：按「知识点覆盖检测」结果补题 + 修复两处引擎/内容缺陷。
//   A. 按课本课时补齐六年级缺口（覆盖率 69% → 97.5%，160 个知识点）：
//      圆柱与圆锥(50%→100%)、百分数一(60%→100%)、圆(67%→100%)、负数(63%→100%)、
//      比例(55%→100%)、行程问题(50%→100%)、位置与方向(75%→100%)、
//      按比分配(0%→100%)、百分数应用题(40%→100%)、展开图(容量 41→378)、
//      视图与展开(20%→100%)、分数除法(70%→91%)、分数乘法(90%)、
//      统计(补条形/折线统计图)、百分数二(补最优购物方案)、工程问题(补中途离开)。
//   B. 修复跨年级超纲：五年级「专项·分数应用题」与六年级「专项·百分数应用题」
//      原共用 g_app_fraction，导致百分数题泄漏到五年级（_audit_cross 实测 10 题次）。
//      现拆分为 g5_app_fraction（只含分数）与 g_app_fraction（分数+百分数）。
