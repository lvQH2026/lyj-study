...
const g3 = KB[3];
ok('三年级存在双轨（含 group:课本）', g3[1].some(u => u.group === '课本') && g3[2].some(u => u.group === '课本'));

const tb1 = g3[1].filter(u => u.group === '课本');
const tb2 = g3[2].filter(u => u.group === '课本');
ok('三上课本同步单元=9', tb1.length === 9, 'actual=' + tb1.length);
ok('三下课本同步单元=8', tb2.length === 8, 'actual=' + tb2.length);
...
ok('所有课本单元含 summary≥3/fidx≥2/method≥2', allMeta, bad.join(','));
...
const sp1 = g3[1].filter(u => u.group === '专项');
const sp2 = g3[2].filter(u => u.group === '专项');
ok('三上专项≥5', sp1.length >= 5, 'actual=' + sp1.length);
ok('三下专项≥5', sp2.length >= 5, 'actual=' + sp2.length);

try { w.eval('examState.grade=3; examState.semester=1; examState.type="final";'); } catch (e) { errs.push('set examState: ' + e.message); }
const ex = w.eval('getExamUnits()');
ok('期末只考课本单元（9个，三上）', ex.units.length === 9, 'actual=' + ex.units.length);
try { w.eval('examState.grade=3; examState.semester=2; examState.type="final";'); } catch (e) {}
const ex2 = w.eval('getExamUnits()');
ok('期末只考课本单元（8个，三下）', ex2.units.length === 8, 'actual=' + ex2.units.length);
