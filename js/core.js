...
// ============================================================
// v84r · 数值 / 分数「精确」比较（修复 parseFloat 宽松比较判定 BUG）
// ------------------------------------------------------------
// 背景（2026-09-02 全站深度测试报告 定位的 P0）：
//   旧判分用 `!isNaN(parseFloat(u)) && parseFloat(u) === parseFloat(c)` 做容差。
//   但 parseFloat 遇到非数字字符就停：
//     · parseFloat("8/5") 与 parseFloat("8/3") 都取到 8 → 「分母写错」也被判对；
//     · 「7个十和4个一」与「7个十和5个一」都取到 7   → 「个位写错」也被判对。
//   孩子答错仍得分 → 成绩虚高、错题不进错题本、家长看到的正确率失真，
//   直接破坏本网站的核心用途（快速、准确测出真实掌握度）。
//
// 规则（分域处理，宁严勿松）：
//   · 两边都是纯数值（含负号 / 小数 / 前导 .5） → 数值比较（1 与 1.0、.5 与 0.5 等价）
//   · 两边都是 a/b 分数          → 交叉相乘判等（自带约分等价：2/4 === 1/2）
//   · 一边分数一边纯数值          → 转小数比较（5/4 === 1.25）
//   · 含中文（计数器 / 带单位 / 概念词）→ 不做数值容差，交回调用方精确字符串比较
//   · 其他（算式 / 含单位英文等）  → 不做数值容差，交回调用方精确字符串比较
//
// 共用：math.js fillAnswerEquals（移动端）与 pc.js pcJudge（PC 端）都调它，
// 避免两处判分口径不一致。
// ============================================================
function looseNumericEquals(u, c) {
  const us = String(u == null ? '' : u).trim();
  const cs = String(c == null ? '' : c).trim();
  if (us === cs) return true;
  if (us === '' || cs === '') return false;

  const NUM_RE = /^-?(\d+(\.\d*)?|\.\d+)$/;   // 12 / 12.5 / .5 / -3
  const FRAC_RE = /^(-?\d+)\s*\/\s*(-?\d+)$/; // 8/5、8 / 5

  // 含中文 → 绝不做数值容差（计数器「7个十和5个一」、带单位「5厘米」等）
  if (/[\u4e00-\u9fa5]/.test(us) || /[\u4e00-\u9fa5]/.test(cs)) return false;

  const UF = us.match(FRAC_RE), CF = cs.match(FRAC_RE);
  if (UF && CF) {
    const a = parseInt(UF[1], 10), b = parseInt(UF[2], 10);
    const x = parseInt(CF[1], 10), y = parseInt(CF[2], 10);
    if (b === 0 || y === 0) return false;
    return a * y === x * b;                    // 交叉相乘，自动等价约分
  }
  const UN = NUM_RE.test(us), CN = NUM_RE.test(cs);
  if (UF && CN) { const b = parseInt(UF[2], 10); return b !== 0 && (parseInt(UF[1], 10) / b) === parseFloat(cs); }
  if (CF && UN) { const y = parseInt(CF[2], 10); return y !== 0 && parseFloat(us) === (parseInt(CF[1], 10) / y); }
  if (UN && CN) return parseFloat(us) === parseFloat(cs);
  return false;                                 // 算式等：不做容差，交回精确字符串比较
}
...
function detectMathConcepts(text) {
  ...
  const rules = [
    ...
    // v84r：去掉裸单字「比」——它会命中「甲比乙多」「比大小」等普通比较句，
    // 导致非比和比例的题被误标（实测分数乘法卷 20 题有 5 题被误标为「比和比例」）。
    { keys: ['比例','比值','按比分配','正比例','反比例','比例尺','化简比','最简比','解比例'], name: '比和比例' },
    ...
    // v84r：去掉裸单字「角」——「3角」是人民币，会与「人民币」规则互撞；
    // 同时补 res 正则，用「数字+度/°」兜住「这个角是45度」这类只有度数没有「角度」二字的题。
    { keys: ['角度','锐角','直角','钝角','平角','周角','量角器','角的度量','三角形内角和','度数','∠'], name: '角与角度', res: [/\d+\s*度/, /\d+\s*°/] },
    ...
    // v84r：去掉裸单字「元/角/分」——「分」命中「分数/分母/约分」、「角」命中「角度/三角形」，
    // 实测分数乘法卷 20 题 15 题、角的度量卷 20 题 20 题被误标为「人民币」（P2 报告项）。
    // 改为「数字 + 元/角/分」的金额语境匹配，并排除 分钟/分数/分之 等非货币用法。
    { keys: ['人民币','购物','找零','付钱','付款','价钱','单价','总价','找回','收银','元角分'], name: '人民币',
      res: [/\d+\s*元/, /\d+\s*角/, /\d+\s*分(?!钟|数|之)/] },
    { keys: ['分米','厘米','毫米','千米','单位换算'], name: '长度单位', res: [/\d+\s*米(?![^\d]|$)/] },
    { keys: ['吨','公斤','单位换算'], name: '质量单位', res: [/\d+\s*千克/, /\d+\s*克(?![^\d]|$)/] },
  ...
  // v84r：规则支持两种命中方式 —— keys（子串）与 res（正则，用于「数字+单位」这类语境匹配）。
  rules.forEach(function (r) {
    let hit = false;
    if (r.keys && r.keys.some(function (k) { return t.indexOf(k) !== -1; })) hit = true;
    if (!hit && r.res && r.res.some(function (re) { return re.test(t); })) hit = true;
    if (hit && out.indexOf(r.name) === -1) out.push(r.name);
  });
  return out;
}
