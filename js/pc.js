...
(function () {
  ...
  PC.runExam = function () {
    ...
    if (t === 'unit') {
      // 单元考：走考试模式（beginQuiz mode=exam），而非单元练习
      if (S.subject === 'math') startMathExam('unit');
      else if (S.subject === 'english') startEngExam('unit');
  ...
  function beginQuiz(questions, mode, title, module, grade, isExam, restart, badge) {
    ...
    if (isExam && window._examTimer) clearInterval(window._examTimer);
    if (isExam) { window._examTimer = setInterval(function() { var el = document.getElementById('pcExamTimer'); if (!el) return; var sec = Math.floor((Date.now() - S.quiz.startTime) / 1000); var m = String(Math.floor(sec/60)).padStart(2,'0'); var ss = String(sec%60).padStart(2,'0'); el.textContent = '⏱ ' + m + ':' + ss; }, 1000); }
    renderQuiz();
  }
  function renderQuiz() {
    ...
    h += '<div class="pc-quiz-head"><div class="pc-quiz-title"><button class="pc-quiz-back" onclick="PC.back()" title="返回上一页">←</button>' + esc(q.title) + '</div><div class="pc-quiz-progress">第 ' + (i + 1) + ' / ' + total + ' 题' + (q.isExam ? '　<span id="pcExamTimer" style="color:#B4945A;font-weight:600">⏱ 00:00</span>' : '') + '</div></div>';
    ...
    if (item.type === 'choice' || item.type === 'judge' || item.type === 'shape_choice') {
    ...
    if (inHint) {
      ...
      if (q.isExam && i < total - 1) h += '<button class="pc-btn gold" onclick="PC.finish()">交卷</button>';
      if (i === total - 1) h += '<button class="pc-btn gold" onclick="PC.finish()">' + (q.isExam ? '交卷' : '查看成绩') + '</button>';
    }
    h += '</div></div>';
    c.innerHTML = h;
    ...
    if (res && !inHint && (item.type === 'choice' || item.type === 'judge' || item.type === 'shape_choice')) {
  ...
  PC.finish = function () {
    if (window._examTimer) { clearInterval(window._examTimer); window._examTimer = null; } finishQuiz(); };
